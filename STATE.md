...
Last updated: 2026-07-07 (app-contracts-registry task)
...
- Phase 2 continue (see plan-invoke-apps.md): B resilience (wall-clock deadline, classify+retry, circuit breaker — AC-2), C app-caller (health/cold-start + single grounded HTTP call), D executor (DAG waves, budget, partial results, web fallback), E grounded synthesis + verbatim voice (AC-4), F trace/observability, G CLI --execute + hermetic e2e.
