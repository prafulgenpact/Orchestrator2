...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        action="store_true",
        help="after the run, print the full observability report (KPIs, cost, quality, alerts, "
        "recent runs, and this run's drill-down)",
    )
...
def _runs_command(argv: list[str]) -> int:
    ...
    p_report = sub.add_parser("report", help="full report: KPIs + alerts + recent runs + a run")
    p_report.add_argument("--run-id", help="also drill into this run at the bottom of the report")
    p_report.add_argument("--limit", type=int, default=10, help="recent runs to list (default 10)")
    ...
    if args.command == "list":
        if args.kpis:
            data = kpis(root)
            print(json.dumps(data, indent=2) if args.json else render_kpis(data))
            return 0
    ...
    if args.command == "report":
        data = kpis(root)
        fired = alerts(root)
        recent = list_runs(root, limit=args.limit)
        detail = get_run(args.run_id, root=root) if args.run_id else None
        if args.json:
            print(
                json.dumps({"kpis": data, "alerts": fired, "recent": recent}, indent=2, default=str)
            )
        else:
            print(render_report(data, fired, recent, detail))
        return 0
...
def _full_report(run_id: str, root: str | None = None) -> str:
    """The combined report for the `--report` flag: whole-store KPIs/alerts/recent plus this run."""
    return render_report(
        kpis(root),
        alerts(root),
        list_runs(root, limit=10),
        get_run(run_id, root=root),
    )
...
def _maybe_report(args: argparse.Namespace, run_id: str) -> None:
    """With --report, print the full observability report after the run. Best-effort: a report
    failure (e.g. an unreadable store) never changes the exit code or hides the answer above."""
    if not args.report:
        return
    try:
        print()
        print(_full_report(run_id))
